...
def sample(args, train_label = None):
    ...
    if args.dataset.endswith("2") or args.dataset.endswith("4"):
        N = train_label.size(0)
        idx = torch.randperm(N, device=train_label.device)[:12000]  # 在同一设备上生成随机索引
    ...
    with torch.no_grad():
        ...
        noisydata = torch.randn(
            ...
            #size=[5, args.input_size, args.window_size], device=device)
