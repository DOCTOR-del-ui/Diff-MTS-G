...
if __name__ == '__main__':
    ...
    args.lr = 2e-4
    args.model_name = "DiffTSmarkovpluswf1i"
    selfeval = False
    
    for idx in [4]:
        for window in [48]:
            args.window_size = window
            args.dataset = f'FD00{idx}'
            
            if not selfeval:
                loaded_data = np.load('./weights/syn_data/syn_'+ args.dataset+'_'+ args.model_name + f'_{args.window_size}ddpm' +'.npz')
                #print(loaded_data["label"], type(loaded_data["label"]))
            ...
            if selfeval:
                loaded_data = {}
                loaded_data["data"] = train_data.clone().detach().cpu().numpy()
                loaded_data["label"] = np.ones((train_data.shape[0], 1))
            
            test_list = []
                
            for i in range(10):
                test_res = discrimative_score_metrics(args, {'data':train_data,'label':train_label}, loaded_data)
                test_list.append(test_res)
                print(f"Run {i+1}/10")
                
            avg_test_acc = sum(test_list) / len(test_list)
            std_test_acc = np.std(test_list, ddof=1)  # 无偏标准差
            print(test_list)
            print(
                f"Average Test Accuracy: {avg_test_acc} ± {std_test_acc:.4f}   "
                f"MIN Test Accuracy: {min(test_list)}"
            )
